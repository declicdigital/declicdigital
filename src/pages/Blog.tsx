...
const VintageCard = ({ article, featured = false }: { article: Article; featured?: boolean }) => {
  ...
  return (
    ...
    >
      <div className={`overflow-hidden relative ${featured ? "aspect-[16/9]" : "aspect-[16/9]"}`}>
        {article.coverImageUrl ? (
          <img src={article.coverImageUrl} alt={article.title}
            className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
            loading="lazy" width={featured ? 1280 : 640} height={featured ? 720 : 360} />
        ) : (
        ...
        <span className={`absolute top-3 left-3 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold shadow-md ${categoryColors[article.category] || "bg-secondary text-secondary-foreground"}`}>
          {article.category}
        </span>
      </div>
      <div className="p-5 md:p-6">
        <h2 className={`font-bold leading-snug group-hover:text-primary transition-colors ${featured ? "text-xl md:text-2xl" : "text-base md:text-lg"}`} style={{ color: "#2B1E3F" }}>
          {article.title}
        </h2>
        <p className="mt-2 text-sm leading-relaxed" style={{ color: "#2B1E3F", opacity: 0.65 }}>{article.excerpt}</p>
        <div className="mt-4 flex flex-wrap gap-1.5">
          {article.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium" style={{ backgroundColor: "#E9F2F4", color: "#2B1E3F" }}>
              <Tag size={9} /> {tag}
            </span>
          ))}
        </div>
  ...
  return (
    <PageLayout hideBlogCarousel>
      ...
      {allCategories.length > 0 && (
        <section className="py-8" style={{ backgroundColor: "#E9F2F4" }}>
          <div className="container">
            ...
            <div className="flex flex-wrap gap-2">
              {allCategories.map((cat) => (
                <Link key={cat} to={`/blog/categorie/${getCategorySlug(cat)}`}
                  className={`rounded-full px-4 py-2 text-sm font-semibold transition-all hover:opacity-80 hover:-translate-y-0.5 ${categoryColors[cat] || "bg-secondary text-secondary-foreground"}`}
                  style={{ border: "1.5px solid rgba(43,30,63,0.12)", boxShadow: "2px 2px 0px rgba(43,30,63,0.10)" }}>
                  {cat}
                </Link>
              ))}
            </div>
