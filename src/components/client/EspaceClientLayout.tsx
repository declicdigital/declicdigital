...
import { LayoutDashboard, MessageSquare, FileText, Receipt, LogOut, Menu, X } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import logo from "@/assets/logo-declic-digital-new.webp";

const navItems = [
  { to: "/espace-client", icon: LayoutDashboard, label: "Mon projet", exact: true },
  { to: "/espace-client/messages", icon: MessageSquare, label: "Messages" },
  { to: "/espace-client/documents", icon: FileText, label: "Documents" },
  { to: "/espace-client/factures", icon: Receipt, label: "Factures" },
];
      ...
      <aside className="hidden lg:flex flex-col w-64 fixed h-full border-r" style={{ background: "hsl(263, 36%, 13%)", borderColor: "rgba(255,255,255,0.07)" }}>
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map((item) => (
            <Link key={item.to} to={item.to}
              ...
              style={isActive(item.to, item.exact)
        ...
        <div className="p-4 border-t" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
          <a href="https://declicdigital.net" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs mb-2 transition-all"
          ...
          <nav className="p-4 space-y-1">
            {navItems.map((item) => (
              <Link key={item.to} to={item.to} onClick={() => setMenuOpen(false)}
                ...
                style={isActive(item.to, item.exact) ? { background: "rgba(255,255,255,0.08)", color: "white" } : { color: "rgba(255,255,255,0.45)" }}>
                <item.icon size={16} /> {item.label}
              </Link>
            ))}
            <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium w-full" style={{ color: "rgba(255,255,255,0.35)" }}>
              <LogOut size={16} /> Déconnexion
            </button>
          </nav>
        </div>
